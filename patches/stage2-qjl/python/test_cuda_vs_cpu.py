...
cpu.ggml_tqp_prepare_query_d128.argtypes = [ctypes.POINTER(ctypes.c_float), ctypes.POINTER(ctypes.c_float)]
cpu.ggml_tqp_prepare_query_d256.restype = None
cpu.ggml_tqp_prepare_query_d256.argtypes = [ctypes.POINTER(ctypes.c_float), ctypes.POINTER(ctypes.c_float)]
...
cuda.tqp_cuda_quantize_row_d128.argtypes = [ctypes.POINTER(ctypes.c_float), ctypes.c_void_p, ctypes.c_int64]
cuda.tqp_cuda_quantize_row_d256.restype = ctypes.c_int
cuda.tqp_cuda_quantize_row_d256.argtypes = [ctypes.POINTER(ctypes.c_float), ctypes.c_void_p, ctypes.c_int64]
...
def _cpu_quantize(d: int, x_np):
    if d == 128:
        blk = block_tq4p_d128()
        cpu.ggml_quantize_row_tq4p_d128(_as_float_ptr(x_np), ctypes.byref(blk), d)
    else:
        blk = block_tq4p_d256()
        cpu.ggml_quantize_row_tq4p_d256(_as_float_ptr(x_np), ctypes.byref(blk), d)
    return blk


def _cuda_quantize(d: int, x_np):
    if d == 128:
        blk = block_tq4p_d128()
        err = cuda.tqp_cuda_quantize_row_d128(_as_float_ptr(x_np), ctypes.byref(blk), d)
    else:
        blk = block_tq4p_d256()
        err = cuda.tqp_cuda_quantize_row_d256(_as_float_ptr(x_np), ctypes.byref(blk), d)
    assert err == 0
    return blk
...
def test_byte_identical_quantize(d, vectors):
    for i, x in enumerate(vectors):
        x_np = x.float().numpy().copy()
        cpu_bytes = _block_bytes(_cpu_quantize(d, x_np))
        cuda_bytes = _block_bytes(_cuda_quantize(d, x_np))
        if cuda_bytes != cpu_bytes:
            ...
            pytest.fail(
                f"byte mismatch on vec {i} d={d}: {len(diffs)} bytes differ, "
                f"first offset {diffs[0]}, fp16_within_ulp={fp16_fields_close}, "
                f"qs_signs_exact={bit_fields_equal}"
            )


def test_prepare_query_agreement(d, constants, vectors):
    for q in vectors[25:35]:
        ...
        if d == 128:
            cpu.ggml_tqp_prepare_query_d128(_as_float_ptr(q_np), cpu_sq)
            err = cuda.tqp_cuda_prepare_query_d128(_as_float_ptr(q_np), cuda_sq, cuda_q_rot)
        else:
            cpu.ggml_tqp_prepare_query_d256(_as_float_ptr(q_np), cpu_sq)
            err = cuda.tqp_cuda_prepare_query_d256(_as_float_ptr(q_np), cuda_sq, cuda_q_rot)

        assert err == 0
        cpu_sq_t = torch.tensor(list(cpu_sq))
        cuda_sq_t = torch.tensor(list(cuda_sq))
        cuda_q_rot_t = torch.tensor(list(cuda_q_rot))
        py_q_rot_t = constants.pi @ q.float()

        assert (cuda_sq_t - cpu_sq_t).abs().max().item() < 1e-5
        assert (cuda_q_rot_t - py_q_rot_t).abs().max().item() < 1e-5


def test_vec_dot_agreement(d, vectors):
    keys = vectors[:25]
    ...
    for q in queries:
        ...
        for k in keys:
            ...
            blk = _cpu_quantize(d, k_np)

            sq = (ctypes.c_float * d)()
            if d == 128:
                cpu.ggml_tqp_prepare_query_d128(_as_float_ptr(q_np), sq)
                ...
                cpu.ggml_tqp_prepare_query_d256(_as_float_ptr(q_np), sq)
    ...
    assert max_abs < 1e-4, f"CUDA vs CPU vec_dot max diff {max_abs:.2e}"


def test_dispatch_wrapper_matches_block_api(d, vectors):
    keys = vectors[:5]
    q = vectors[5]
    q_np = q.float().numpy().copy()
    ...
    if d == 128:
        ...
        blocks = BlockArray(*[_cpu_quantize(d, k.float().numpy().copy()) for k in keys])
