// TurboQuant-ish quantization types for ggml (WHT variant).
//
// Adds two block quantization types that implement a variant of the
// two-stage algorithm from Zandieh et al., "TurboQuant: Online Vector
...
//
// Stage 1: Randomized Hadamard Transform Π = (1/√d) · H · diag(σ) with a
//          per-layer ±1 sign vector σ (replacing the paper's Haar random
//          orthogonal rotation) + per-coordinate 3-bit Lloyd-Max scalar
//          quantization.
...
//
// This branch intentionally diverges from the paper: the Haar rotation is
// swapped for a Randomized Hadamard Transform, so apply cost drops from
// O(d²) to O(d log d) and per-layer storage drops from d² floats to d.
// The algorithm remains TurboQuant-ish (same two-stage structure) but
// stops being paper-exact.
//
// This is still NOT the existing TQ3_0 / TQ3_1S / TQ3_4S path in this
// fork — those use a signed Walsh-Hadamard rotation with no QJL stage.
// This type keeps the QJL residual-sign estimator.
//
...
//
// Constants σ, S, and Lloyd-Max centroids are generated by
// patches/stage2-qjl/python/generate_constants.py. Per-layer σ_i and S_i
// are keyed by layer index (seeds: σ_i=42+i, S_i=43+i). 32 layers
// supported. The layer index is stored in each block header as a single
// byte.
...
typedef struct {
    ...
    uint8_t  layer_idx;       // layer index [0, 31]; selects per-layer σ and S
...
// `layer_idx` selects the per-layer σ sign vector and S matrix (0..31).
