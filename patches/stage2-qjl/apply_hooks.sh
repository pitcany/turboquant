...
staging_code = (
    ...
    f'{indent}// Mirror SET_ROWS: find-before-put so repeated CPYs on the same dst_d\n'
    f'{indent}// reuse the existing staging buffer instead of leaking pool allocs\n'
    f'{indent}// and evicting valid entries from the 4-slot cache.\n'
    f'{indent}{{\n'
    f'{indent}\tconst int dev = ggml_cuda_get_device();\n'
    f'{indent}\thalf * stg = tqp_staging_find(dev, dst_d);\n'
    f'{indent}\tif (!stg) {{\n'
    f'{indent}\t\tsize_t stg_bytes = 0;\n'
    f'{indent}\t\tstg = (half *)ctx.pool().alloc(ne * sizeof(half), &stg_bytes);\n'
    f'{indent}\t\ttqp_staging_put(dev, dst_d, stg, stg_bytes, ne);\n'
    f'{indent}\t}}\n'
